...
def download_batches5(souls, conn, batch_size, tmp_): # get
	"""Executes the queries to find the content for the notes that do not exist locally, or whose contents do not exist locally. Takes the list of 
	dictionaries from contrast and makes them into queries for the given file[s]. *Executemany() cannot be used with SELECT; it is for DML quries only.
	This function passes the found data to the wr_data function, which writes the new data structure to the disk.
	"""
		...
		with conn.cursor() as cursor:
			try:
				...
				if batch:
					...
					logger.info(f"Wrote batch in {(end - beg):.4f} seconds.")

					wr_batches(batch, tmp_)
					beg = time.perf_counter()
					logger.info(f"Wrote batch in {(beg - end):.4f} seconds.")
				...
				logger.info('Batched download completed w.o exception.')
...
# def calc_compression(before, after): # t_size [uncompressed], ct_size [compressed]
...
# 				logger.info(f"Disk space [in gb] saved by compression: {comp_gb:.4f}.")
# 			else:
# 				logger.info(f"Disk space [in mb] saved by compression: {comp_mb:.4f}.")
# 		else:
# 			logger.info(f"Disk space [in kb] saved by compression: {comp_kb:.4f}.")
# 	else:
# 		logger.info(f"Disk space [in bytes] saved by compression: {compressed:.4f}.")
