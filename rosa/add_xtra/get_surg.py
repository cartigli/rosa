...
def main(args):
        ...
        try:
            ...
            if ratio > 80:
                with phones() as conn:
                    ...
                    logger.info(f"batch size returned: {batch_size}")
                    ...
                    if gates:
                        ...
                        mk_rrdir(lgates, patient) # write directory heirarchy to tmp_ directory
                        logger.info('new directories [gates] written to disk')

                    # if caves:
                    #     logger.debug(f"Ignoring local-only directories [caves].")

                    # with phones() as conn: # phones keeps hanging up!
    ...
    logger.info('[get] [surgic] complete')
