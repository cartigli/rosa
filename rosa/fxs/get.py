...
def main(args=None):
    ...
    indexed_dirs = scrape_dindex(index)
    ...
    if xdiff is True:
        ...
        with fat_boy(LOCAL_DIR) as (tmp_, backup):
            ...
            originals(diffs, tmp_, index.parent)

            for r in remaining:
                diffs.append(r)

        refresh_index(diffs, index)
        ...
        # refresh_index won't break the system if it fails, but it will force it out of sync and need to be resynced. The local 
        # copy of the directory is untouched, so we don't need a backup of the orginal like fat_boy uses, but it will require 
        # retry on failure (or perfect code, which is unlikely). This also means refresh_index should get a list of modified, 
        # deleted, and unchanged files to update in the index.
